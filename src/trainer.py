...
def model_train(train_data, val_data, test_data, con_data, model_joint, args, logger, pretrain_flag):
    ...
    with torch.no_grad():
        ...
        # Source-next 分支：
        # Source 使用 source_popular_ratio
        # Target 使用 popular_ratio
        if pretrain_flag:
        ...
        for start_idx in range(
            ...
            # Ground truth 是否屬於目前 domain 的 Popular
            ...
            # 分別累積 Popular、Unpopular-seen、Unseen
            # 的 HR / NDCG
            for metric_name, values in sample_metrics.items():
