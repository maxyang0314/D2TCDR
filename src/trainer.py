...
def build_target_next_frequency_weights(
    train_data,
    alpha=1.0
):
    """
    ...
    只統計 Target training data 的 next frequency。
...
def model_train(train_data, val_data, test_data, con_data, model_joint, args, logger, pretrain_flag):
    ...
    if not pretrain_flag:
        ...
        (
            ...
            target_next_counter
        ) = build_target_next_frequency_weights(
            train_data,
        ...
        next_reweight_info = {
            ...
            'Observed Next Items':
                len(target_next_counter),

            'Max Next Frequency':
                max(
                    target_next_counter.values()
                ),
    ...
    for epoch_temp in range(epochs):
        ...
        for j in range(num_batches):
            ...
            # ============================================
            # 2. Target Next Frequency-aware Reweighting
            #    只作用在 Stage-2
            # ============================================
            if not pretrain_flag:
                ...
                # Stage-1 Source 完全不做 reweighting
            ...
            # ============================================
            # 4. Stage-1 Target condition
            # ============================================
            if pretrain_flag:

                con_batch = con_data.sample(
                    n=args.batch_size
                ).to_dict()
            ...
            # 5. Forward
            # ============================================
            scores, diffu_rep, weights, t, \
            item_rep_dis, seq_rep_dis, em_loss = model_joint(
                seq,
                target,
                con_seq,
                pretrain_flag,
            ...
            # ============================================
            # 6. Recommendation Loss
            # ============================================
            loss_diffu_value = model_joint.loss_diffu_ce(
                diffu_rep,
                target,
                pretrain_flag,
                sample_weights=target_next_weights
            )
